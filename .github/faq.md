...
Не работает: [Добавьте отчет](https://github.com/localzet/shadowsocks-android/issues/new) прикрепив лог (logcat), или отправьте отзыв в Rustore. Затем попробуйте стереть данные приложения.
...
SHADOWSOCKS_UID=`dumpsys package com.localzet.shadowsocks | grep userId | cut -d= -f2 - | cut -d' ' -f1 -`
PORT_DNS=5450
...
$IP6TABLES -F
```
4. Убедитесь, что трафик для Shadowsocks разрешен;
